...
struct NonMP4ScannerView: View {
    @StateObject private var viewModel = NonMP4ScannerViewModel()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text("Scan a folder (including subfolders) and quickly find video files that are not MP4.")
                .font(.caption)
            ...
            GroupBox("Scan Results") {
                if viewModel.results.isEmpty {
                    VStack(spacing: 12) {
                        ...
                        Text("Run a scan to list video files and flag non-MP4 items.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                        ...
                        if displayedResults.isEmpty {
                            ...
                            List(displayedResults) { result in
                                HStack(spacing: 12) {
                                    ...
                                    Text(result.fileExtension.uppercased())
                                        .font(.caption2)
    ...
    private var displayedResults: [NonMP4ScanResult] {
